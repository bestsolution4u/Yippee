...
function process_maneuver_triggers()
    ...
    elseif trigger == "ANNOUNCEMENT" then
        ...
        elseif road_class == "STREET"
            ...
            table.insert(result_list, distance_to_maneuver_follow )
...
function get_distances_and_units()
    ...
    if tonumber(maneuver_1.dist_to_unit) == 1 and maneuver_1.dist_unit == "KILOMETER" then
        ...
        distance_to_maneuver_after = misc[ "one_km_wav" ]
    elseif tonumber(maneuver_1.dist_to_unit) == 1 and maneuver_1.dist_unit == "MILE" then
        
        distance_to_maneuver_after = misc[ "one_mile_wav" ]
    else
        distance_to_maneuver_after = distances[ tonumber(maneuver_1.dist_to_unit) ]
    end

     
    distance_to_maneuver_follow = distances_follow[ tonumber(maneuver_1.dist_to_unit) ]
    
    unit_to_maneuver_up = unit_after[ maneuver_1.dist_unit ] [ up ]

    
    if tonumber(maneuver_1.dist_to_unit) <= 4 and maneuver_1.dist_unit == "KILOMETERS" then
        
        unit_to_maneuver_down = misc[ "kilometry_wav" ]
    elseif tonumber(maneuver_1.dist_to_unit) >= 5 and maneuver_1.dist_unit == "KILOMETERS" then
        
        unit_to_maneuver_down = misc[ "kilometrow_wav" ]
    elseif tonumber(maneuver_1.dist_to_unit) <= 4 and maneuver_1.dist_unit == "MILES" then
        
        unit_to_maneuver_down = misc[ "mile_wav" ]
    elseif tonumber(maneuver_1.dist_to_unit) >= 5 and maneuver_1.dist_unit == "MILES" then
        
        unit_to_maneuver_down = misc[ "mil_wav" ]
    else
        unit_to_maneuver_down = unit_follow[ maneuver_1.dist_unit ] [2]
    end
...
function add_maneuver_to_table( include_distance )
    if include_distance == true then
        ...
        table.insert( result_list, distance_to_maneuver_after )
        
        table.insert( result_list, unit_to_maneuver_up )
    end
