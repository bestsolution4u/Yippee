...
function 	push_traffic_light_term( trigger, maneuver )
    if voice_settings["nguidance_trafficlights"] == true then
				if maneuver.ng_type =="TRAFFIC LIGHT" and trigger ~= "COMMAND" then
					key = "traffic_light_" .. maneuver.traffic_light_int
					if _G.misc[ key ] then
							table.insert( _G.result_list, _G.misc[ key ] )
					else
							table.insert( _G.result_list, "missing "..key )
					end
